...
select * from [dbo].[tblEmployee] where [EmployeeNumber] = 134
...
---  every time we run we get new plan guide
GO
...
--When to use cursors
-- declared the cursor based on that statement
-- to write in a few line many select statements
...
close csr
deallocate csr -- deallocate the memory needed for the cursor
-- Dissadvantage -- needs a lot of time.
...
-- Seek is faster than scan because it uses indices
